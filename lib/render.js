...
module.exports = {
    ...
    //开始渲染
    startRender: function (viewModel, viewPath, callback) {
        getViewSection(viewPath).then(function (html) {
            callback(renderContext(html, viewModel));
            console.log('render complated');
        }).catch((viewPah) => {
            callback(renderContext(readFile(viewPath), viewModel));
        });
    }
}