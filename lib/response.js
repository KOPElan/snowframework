...
function sendView(controller, action, res) {
    ...
    readStream(tempPath, res);
}

//返回文件
function sendFile(filePath, res) {
    readStream(filePath, res);
}
...
function sendContent(data, contentType, res) {
 ...
 //返回错误页面
 function sendBadRequest(error, res) {
    if (appsetting.environment == 'development') {
        createResponse(500, this.contentType.txt, error.message + error.stack, res);
    } else {
        readStream(path.join(appsetting.router.views, appsetting.router.error), res);
    }
}
